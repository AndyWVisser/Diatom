function [dBdt]= equdyn(t,B,D,G,r,T,seasonal) 
...
%===Trophic interactions===
 Fd = zeros(1,Imax);
 D.m = zeros(size(D.v));     
 Bpred = sum(G.b.*G.phif);   %the biomass of predators [mg C / m-3] in size classes r(i)
...
%===Grazing by copepods===
 ...
 mpmask = x > 10^(-2);
 D.mp = mp0*(1-palat)*Zday*mpmask.*D.r.^(-3/4);
 G.mp = mp0*Zday*mpmask.*G.r.^(-3/4);
...
% dBdt_d = (D.g.*Bd-D.mp.*Bd-D.m.*Bd -m2.*Bd.^2 -dilday.*Bd); %dynamic equation for diatoms' biomass %matrix Jmax x Imax
% dBdt_g = (G.g.*Bg-G.mp.*Bg-G.m.*Bg -m2.*Bg.^2 -dilday.*Bg);%dynamic equation for generalists' biomass
